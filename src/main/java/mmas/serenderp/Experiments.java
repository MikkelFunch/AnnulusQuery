...
public class Experiments {
	...
	public static double suggestInnerAnnulusRadius(Map<String, SparseVector> movies) {
		...
		String movie = null;
		for(int i = 0; 0 < moviePairs.length; i++) {
			if(movies.containsKey(moviePairs[i])) System.out.println(moviePairs[i] + " is not in movies.");
			if(i%2 == 0) {
				movie = moviePairs[i];
			} else {
