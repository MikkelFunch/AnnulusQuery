...
public class Magic {
	public static int numberOfRatedMoviesForEligibility = 500;
...
//>>>>>>> 44a7484ea060cc07d8036a29143ce67f683b16c1
			
			public static List<Integer> getUserQueryPoints(List<Entry<Integer, Double>> userRatings) {
	List<Entry<Integer, Double>> speedList = new ArrayList<>();
	double maxRating = Integer.MIN_VALUE;
	
	for (Entry<Integer, Double> e : userRatings) {
		...
		for (Entry<Integer, Double> e : speedList) {
			if (e.getValue() == maxRating) {
				maxRatings.add(e.getKey());
			}
		}
...
//		ratingDistancesMap.forEach((k,v) -> System.out.println(k + " " + v/userSample.size()));
	...
	/***
	 * Returns less than zero if the user did not rate any of the
	 * recommendations
	 */
	public static double calculateSerendipityForUser(Map<Integer, Double> userRatings, List<Integer> recommendations) {
		...
		for (Integer movieId : recommendations) {
			if (userRatings.containsKey(movieId)) {
				if (userRatings.get(movieId) > averageRating)
					numberOfUsefulMovies++;
	...
	public static boolean isUserEligible(List<Entry<Integer, Double>> userRatings) {
